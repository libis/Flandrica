...
function exhibit_builder_exhibit_form_item($item, $orderOnForm = null, $label = null, $includeCaption = true)
{
    ...
    if ($item and $item->exists()) {
        ...
        $html .= "<p class='explanation'>Kies een thumbnail die op de  voorpagina deze tentoonstelling zal vertegenwoordigen.</p>";
        $html .= "Als u er geen kiest wordt gewoon de eerst gevonden afbeelding gebruikt.</p>";
        $html .= "<br>";

        //JORIS HACK ->enables choice of pid's
        $values = array();
        $values = $values + digitool_get_thumbnail_array($item);

        if(sizeof($values) != 0){
			$html .= "<div class='radio' style='width:160px;height:400px;float:left;overflow:scroll;'>";
		    //echo radio(array('name'=>'thumbnail', 'id'=>'thumbnail'),$values);
		    foreach($values as $key=>$value){
		     	if($key == exhibit_builder_get_current_page()->ExhibitPageEntry[(int) $orderOnForm]->thumbnail)
		       		$html .= "<input checked type='radio' name='Thumbnail[".$orderOnForm."]' value='".$key."'>";
		        else
		           	$html .= "<input type='radio' name='Thumbnail[".$orderOnForm."]' value='".$key."'>";
	                $html .= $value;
	            }
	        $html .= '</div>';
        }else{
