    ...
    <form id="exhibit-metadata-form" method="post" class="exhibit-builder">
        ...
        <fieldset>
            ...
            <div class="field">
                ...
                <?php
                if($actionName != "Add"){
                	echo '<p class="explanation">Kies een thumbnail die op de  voorpagina deze tentoonstelling zal vertegenwoordigen.</p>';
                	//get items of exhibit
                	$items = get_items(array('exhibit' => $exhibit),40);
                	$values = array();
                	foreach ($items as $item){
						$values = $values + digitool_get_thumbnail_array($item);
                	}
	                ?>
	                <?php if(sizeof($values) != 0){?>
	                <div class="radio" style="width:500px;height:400px;overflow:scroll;">
	                <?php //echo radio(array('name'=>'thumbnail', 'id'=>'thumbnail'),$values);
	                	foreach($values as $key=>$value){
	                		if($key == $exhibit->thumbnail)
	                			echo "<input checked type='radio' name='thumbnail' value='".$key."'>";
	                		else
	                			echo "<input type='radio' name='thumbnail' value='".$key."'>";

	                		echo $value;
	                	}
                	}else{
                		echo "<p>Er zijn nog geen objecten toegewezen aan deze tentoonstelling/rondleiding</p>";
               		}
                ?>
                </div>
            <?php }else{
               		echo '<p class="explanation">U kan pas een  thumbnail kiezen als er paginas met hun objecten zijn toegevoegd aan deze rondleiding.</p>';
              	} ?>
            </div>
            <div class="field">
...
<script>
	jQuery(document).ready(function($){
		jQuery("label img").live("click", function() {
			jQuery("#" + $(this).parents("label").attr("for")).click();
			});
