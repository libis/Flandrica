...
function _define_web_root()
{
    ...
    if ($dir = trim(dirname($_SERVER['SCRIPT_NAME']), '\,/')) {
    	    /* Added by Sam. Evaluating the $_SERVER['SCRIPT_NAME'] is not sufficient if mod_rewrite is used the root .htaccess (dynamic virtual host) */
		$script_dir = explode('/',trim(dirname($_SERVER['SCRIPT_NAME']), '\,/'));
		$request_dir = explode('/',trim($_SERVER['REQUEST_URI'], '\,/'));
		$i = 0;
		$dir ='';
		foreach ($script_dir as $dirp){
			if ($dirp == $request_dir[$i]){
				$dir .= $dirp."/";
				++$i;
			}
		}
